...
There are currently 3 different types of grids, each which implements the `Grid<T>` trait:
...
The `Grid<T>` trait also implements a lot of functions for manipulating the grids.
...
`VecGrid<T>` and `ConstGrid<T, W, H>` specifically, any of these that operate over
rectangular regions *should* be perfectly safe, since only the region where they provided
rectangles overlap with the grid should be scanned. But with `SparseGrid<T>`,
...
removed if `SparseGrid<T>` specifically just panicked if you tried to retrieve
...
might implement `Grid<T>`, which the algorithms cannot account for.
